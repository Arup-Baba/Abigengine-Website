...
import type { PageName, Service, Reel, CartItem, UserDetails, SelectionStep, Review, Testimonial, TyreSpec, VariantDetails, FlatCarDataEntry, Order, Banner } from './types.ts';
...
export const showPage = (pageName: PageName) => {
    ...
    const checkoutPages: PageName[] = ['my-order', 'order-details', 'payment-method', 'order-confirmation'];
    if (!state.isLoggedIn && !checkoutPages.includes(pageName)) {
        state.setUserDetails(null);
    }
    
    // Update nav links active state
    const servicePages: PageName[] = ['car-wash', 'battery-replacement', 'tyre-replacement', 'car-care'];
...
function createStarRatingDisplay(rating: number, reviewCount: number, type: 'summary' | 'full' = 'summary'): string {
...
function renderReviewsList(reviews: Review[]): string {
...
function createPriceDisplay(price: string, mrp?: string): { html: string, discount: number } {
...
const createReelCard = (reel: Reel): HTMLElement => {
...
const createTestimonialCard = (testimonial: Testimonial): HTMLElement => {
    const card = document.createElement('div');
...
const createBannerItem = (banner: Banner): HTMLElement => {
...
const createServiceListCard = (service: Service): HTMLElement => {
    const card = document.createElement('div');
    ...
    const featuresPreview = (service.includedFeatures && service.includedFeatures.length > 0)
        ...
        : (Array.isArray(service.features) ? service.features.map(f => ({ name: f as string })) : []);
    ...
    if (!isVideo) {
        const img = card.querySelector<HTMLImageElement>('.media-img');
...
const createBatteryProductCard = (service: Service): HTMLElement => {
    const card = document.createElement('div');
    ...
    const mainImage = card.querySelector<HTMLImageElement>('.main-image');
    const thumbnails = card.querySelectorAll<HTMLImageElement>('.thumbnail');
...
const createTyreCard = (service: Service): HTMLElement => {
    const card = document.createElement('div');
    ...
    if (images.length > 0) {
        ...
        productThumbnailHtml = `<div class="main-image-container tyre-brand-logo"><img src="" alt="${service.title} thumbnail"></div>`;
    }
        ...
        <div class="tyre-card-details">
            ...
            <div class="tyre-card-tab-content">
                <div class="tab-pane active" data-tab-content="features">
                    <ul class="features">
                        ${(features as string[]).map(f => `<li>${f}</li>`).join('')}
                    </ul>
                </div>
    ...
    const mainImage = card.querySelector<HTMLImageElement>('.main-gallery-image');
    const thumbnails = card.querySelectorAll<HTMLDivElement>('.thumbnail-wrapper');
    const tabs = card.querySelectorAll<HTMLButtonElement>('.tab-button');
    const panes = card.querySelectorAll<HTMLDivElement>('.tab-pane');
    const productThumbnailImg = card.querySelector<HTMLImageElement>('.tyre-brand-logo img');
    const gallery = card.querySelector<HTMLElement>('.tyre-card-gallery');
    ...
    if (mainImage && images.length > 0) {
        ...
        thumbnails.forEach((thumb) => {
            const thumbImg = thumb.querySelector<HTMLImageElement>('.thumbnail-image');
...
const createCartItemComponent = (item: CartItem): HTMLElement => {
    ...
    if (!isVideo) {
        const img = itemElement.querySelector<HTMLImageElement>('img');
...
export const renderBannersSection = () => {
    ...
    if (state.banners.length > 0) {
        ...
        const bannerProgressBar = document.getElementById('banner-progress-bar') as HTMLElement;
...
export function showProfileView(view: 'dashboard' | 'edit') {
    ...
    if (view === 'dashboard') {
        ...
        (modalBody as HTMLElement).style.padding = '0';
        ...
        (modalBody as HTMLElement).style.padding = '20px 30px';
...
export function openAuthModal() {
    ...
    if (!modal) return;
    (document.getElementById('auth-mobile-input') as HTMLInputElement).value = '';
    document.querySelectorAll<HTMLInputElement>('.otp-input').forEach(input => input.value = '');
    showAuthStep('mobile');
...
export function updateCarSelectorDisplay() {
    ...
    const infoContainer = carSelectorTrigger.querySelector('.car-info') as HTMLElement;
    if (!infoContainer) return;

    let carInfo: { carBrandModel: string, brand: string, model: string } | null = null;
    ...
    if (carInfo && carInfo.carBrandModel && Array.isArray(state.carDatabase)) {
        ...
        const thumb = infoContainer.querySelector<HTMLImageElement>('.car-thumb');
...
export const updateProfileUI = () => {
    ...
    if (state.isLoggedIn && state.userDetails) {
        ...
        (document.getElementById('first-name') as HTMLInputElement).value = state.userDetails.firstName || '';
        (document.getElementById('last-name') as HTMLInputElement).value = state.userDetails.lastName || '';
        (document.getElementById('mobile-number') as HTMLInputElement).value = state.userDetails.mobile || '';
        (document.getElementById('car-details') as HTMLInputElement).value = state.userDetails.carBrandModel || '';
        (document.getElementById('car-number') as HTMLInputElement).value = state.userDetails.carNumber || '';
        (document.getElementById('address-street') as HTMLInputElement).value = state.userDetails.street || '';
        (document.getElementById('address-city') as HTMLInputElement).value = state.userDetails.city || '';
        (document.getElementById('address-pincode') as HTMLInputElement).value = state.userDetails.pincode || '';
...
const updateQuantity = (cartItemId: string, change: number) => {
    ...
    if (item) {
        ...
        if (item.quantity <= 0) {
            ...
            if (itemElement) {
                (itemElement.querySelector('.quantity-input') as HTMLInputElement).value = String(item.quantity);
            }
            updateOrderSummary();
            updateCartCountBadge();
        }
...
const renderCheckoutProgressBar = (activeStep: number) => {
...
export const renderPaymentMethodPage = () => {
    ...
    document.getElementById('place-order-btn')?.addEventListener('click', async () => {
        const selectedMethod = (document.querySelector('input[name="payment-method"]:checked') as HTMLInputElement).value;
...
export const renderMyOrdersListPage = () => {
    ...
    container.querySelectorAll<HTMLImageElement>('.order-item-row img[data-src]').forEach(img => {
...
export const openServiceDetailModal = (serviceId: string) => {
    ...
    modalBody.querySelectorAll<HTMLElement>('.carousel-item, .included-item').forEach(parent => {
...
export const openBookingModal = (serviceId: string) => {
    ...
    if(titleEl) (titleEl as HTMLElement).textContent = `Book Slot for: ${service.title}`;
    ...
    const dateInput = (modal.querySelector('#booking-date-input') as HTMLInputElement);
...
export const openReviewModal = (serviceId: string, reviewToEdit?: Review) => {
    ...
    form.reset();
    (document.getElementById('review-id-input') as HTMLInputElement).value = reviewToEdit?.reviewId || '';
    (document.getElementById('review-comment-input') as HTMLTextAreaElement).value = reviewToEdit?.comment || '';

    // Set star rating
    const rating = reviewToEdit?.rating || 0;
    (document.getElementById('review-rating-value') as HTMLInputElement).value = String(rating);
    modal.querySelectorAll('.interactive-star-rating i').forEach(star => {
        const starRating = parseInt((star as HTMLElement).dataset.rating || '0');
...
export const openOrderDetailModal = (orderId: string) => {
...
const createGridItem = (id: string, name: string, imageUrl: string, type: 'logo' | 'car'): HTMLElement => {
...
const createListItem = (id: string, name: string): HTMLElement => {
    const item = document.createElement('div');
...
export const updateCarView = () => {
    ...
    const progressSteps = carSelectionModal.querySelectorAll<HTMLElement>('.progress-step');
    ...
    searchInput.value = '';
    const items = stepContentContainer.querySelectorAll<HTMLElement>('.grid-item, .list-item');
    items?.forEach(item => item.style.display = '');
};

export const goBackCar = () => {
    const stepOrder: SelectionStep[] = ['brand', 'model', 'variant', 'confirmation'];
...
export const handleConfirmation = () => {
    ...
    document.getElementById('confirm-car-btn')?.addEventListener('click', async () => {
        ...
        const carDetails = {
            ...
            selectedVariant: { ...currentVariant, brand: brand!, model: model! }
        };
